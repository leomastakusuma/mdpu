...
class Mydb_Db_Kendaraan extends Mydb_Db_Abstract {
    ...
    public function getAllKendaraanByCabang($id_user=null, $id_cabang=null) {
        $select = $this->select();
    ...
    public function getByidCostumer($nik_costumer){
        ...
        $select->from(array('ken' => 'kendaraan'), array('*'));
        $select->setIntegrityCheck(false);
        $select->join(array('cos' => 'costumer'), 'cos.nik_costumer=ken.nik_costumer', array('nama'));
        $select->where('ken.nik_costumer = ?',$nik_costumer);
        return $this->getAdapterSelect()->fetchAll($select);
    }
