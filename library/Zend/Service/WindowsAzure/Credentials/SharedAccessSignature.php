    ...
    extends Zend_Service_WindowsAzure_Credentials_CredentialsAbstract
{
	 ...
	 * 
	 * Warning: the signed access signature URL must match the user-resource name of the
    ...
    public function setPermissionSet($value = array())
	{
		foreach ($value as $url) {
			if (strpos($url, $this->_accountName) === false) {
				...
				throw new Zend_Service_WindowsAzure_Exception('The permission set can only contain URLs for the user-resource name specified in the Zend_Service_WindowsAzure_Credentials_SharedAccessSignature instance.');
			}
		}
